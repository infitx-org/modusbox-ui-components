...
app.use(function(req, res, next) {
	...
	res.header('Access-Control-Allow-Headers', 'Content-Type, Access-Control-Allow-Credentials, Access-Control-Allow-Headers, Authorization, X-Requested-With');	
	next();
});
...
.listen(port, function (error) {
	if (error) {
		...
		console.info('\n🌎  Listening on port %s.\n Open up \x1b[31m\x1b[1mhttp://localhost:%s/\x1b[0m in your browser.', port, port + appBaseName)
	}
})
