		...
		return (
			<div style={ style } className="data-list-body-cell data-list-body-column-cell" ref='cell'>				
				<div className='data-list-body-cell-content-wrapper'> 
					{ icon && <Icon size={size || 16} name={name || ''} color={color || '#333'} className="data-list-body-cell-icon" /> }
					{ isContent 
						? <div className="data-list-body-cell-content"> { cellContent } </div>
						: <div className="data-list-body-cell-text" ref='box' onClick={ onClick }> { cellContent } </div>
					}
				</div>
			</div>
		)
...
class HeaderCell extends React.Component { 
	...
	changeColumnWidthStart(event){
		...
		let name = this.props.content ? this.props.content( this.props.column ) : this.props.showLabel ? this.props.label : ''  
	...
	render() { 
		...
		const { id, isLastColumn, resizable, searchable, style, column, sortable, label, content, showLabel, isSearching, isSorting, isSortingAsc, onColumnClick, onSearchClick, onSearchChange, onSearchRemove } = this.props
		const labelContent = content ? content( column ) : showLabel ? label : '' 
		const isLabelEmpty = labelContent === ''
		const sortingArrowIconClassName = [ 
			'data-list-header-cell-arrowIcon',
		...
		const sortingArrowIconBoxClassName = [
			'data-list-header-cell-arrowIcon-box',			
			( isLabelEmpty ) && 'center'
		]
		.filter( x => typeof x  === 'string' )
		.join(' ')
		...
		const allowResizeWidth = resizable && ( ! isSearching ) && ! ( typeof style.width === 'number' && style.width < 50 || isLastColumn )
		...
		return (
			...
			>
				<div className='data-list-header-cell-box'>
					{ searchable && 
						<div className='data-list-header-cell-searchIcon-box' onClick={ (isSearching) ? onRemoveFilter : onSearchClick }>
							<Icon size={16}  name='search-small' className={'data-list-header-cell-searchIcon ' + ( isSearching ? 'rotated' : '')} />
						</div>
					}
					{ isSearching && 
						<div className='data-list-header-cell-searchInput-box'>
							<input 
								...
								ref='search-input'
							/>
							<Icon  size={16}  name='close-small' className='data-list-header-cell-searchInput-remove' onClick={ onRemoveFilter } />
						</div>	
					}
					{ ! isSearching && ! isLabelEmpty && 
						<div className='data-list-header-cell-label' ref='box'> { labelContent } </div>
					}
...
const ScrollBarCell = () => {
	return (
		<div className='data-list-body-cell data-list-scrollbar-cell'/>
	)
}

const ScrollBarHeaderCell = () => {
	return (
		<div className='data-list-header-cell data-list-scrollbar-cell' />
	)
}
