...
const OverlayColumnResizer = (props) => (
	<div className={`overlay-column-resizer ${props.isMinimumWidth && 'minimum'}`} style={{ left:props.start, width:props.stop }} > 		
...
const Page = ({ onClick, label, value, isSelected }) => (
	<div
		onClick={onClick}
		className={`paging-page ${isSelected && 'selected'}`}
	>
		{ label }
	</div>
)
...
class ListItems extends React.Component {
	...
	componentWillReceiveProps( nextProps ){	
		...
		if( multiSelectChanged ){	
			this.setState({ forceRenderItems: true, multiSelected: nextProps.multiSelected })			
		}		
		if( forceUpdateList ){
			this.setState({ isLoadingNewData: true, apiError: false, multiSelected: [] })
	...
	getData( cfg = {}, isPartialRequest = false ){		
		...
		.then((json) => {
			...
			if( this.props.wrapper )
				return this.props.wrapper( json )
			else
				return json
		})
		.then( (data) => {		
	...
	getSortingKey( label ){
		...
		if( label != undefined ){
			...
			if( columns.length ){
				if( columns[0].sortable === false) 
					sortingKey = false
				else
					sortingKey = columns[0].key
			}
		}
	...
	getFilteringColumn( label ){
		...
		// if the label is a string, get the column for that label and set the item key 
		if( label != undefined )
			return find( this.props.columns, { label } )
		else
			return undefined
	}	
	...
	filterList( list, filters ){		
		...
		return filters.reduce( (list, filter) => {
			...
			const chunks = pattern.toLowerCase().split(' ').filter( c => c != '' )
			const filtered = list.filter( item => { 
				const field = item[ key ]
				const value = ( field != undefined ) ? ( column.func != undefined ) ? column.func( field, item ) : field : ''
				return chunks.every( chunk => value.toString().toLowerCase().includes( chunk ) );			
			})
		...
		if( isScrollingDown ){		
			...
			if( pixelsToEnd < 1000 && ( this.state.endOfData != this.state.currentIndexStop ) ){
				...
				if( this.state.list.length >= this.state.currentIndexStop + this.state.itemsNumber){
					let data = this.state.list.slice( this.state.currentIndexStop, this.state.currentIndexStop + this.state.itemsNumber)
	...
	handleStopResizeColumnWidth( event ){
		if( this.state.isResizingColumn ){
			...
			let style = {
				...
				dataColumn: { width: 'calc(' + columnPerc + '%' + ' - ' + columnPxToRemove + 'px )' }
			}				
	...
	detectMainMultiselected(){
		...
		return allItemIds.length > 0 && allItemIds.length === allSelectedIds.length && allItemIds.every( (id) => allSelectedIds.includes(id) )	
	}
	...
	render(){
		...
		const isNoDataInfiniteUnfiltered = allowFilter && currentList.length === 0 && ( ! isLoading ) && ( ! apiError ) && filters.length == 0
		const isNoDataInfiniteFiltered = allowFilter && currentList.length === 0 && ( ! isLoading ) && ( ! apiError ) && filters.length > 0
		...
		return (
			<div className='data-list-wrapper' ref='datalist' onMouseMove={ this.handleResizeColumnWidth } onClick={ this.handleStopResizeColumnWidth }>
				{ /*Resize detector*/ }
				...
				<div className='data-list' id={ this.props.id } style={{ display: hideList ? 'none' : 'block' }} >
					...
					<div className='central-box' style={{ display: isSortingOrFiltering ? 'none' : 'flex' }} >
						...
						<div ref='scroller' className='data-list-body-box' style={{ width: this.state.bodyWidth, paddingRight: this.state.scrollbarWidth, display: this.state.isLoadingNewData ? 'hidden' : 'block'}} >
						
							{ /* this element is used to compensate the difference of missing elements to keep the same scrollbar position */ }
							<div id='compensator' style ={{height: this.state.compensatorIndex, width:'100%', background:'#fff'}} />
...
const DataList = ({ 
	...
	const fixedPxAmount = scrollbarWidth + multiSelectWidth + arrowCellWidth + columns.reduce((a,b) => b.width ? b.width + a : a, 0)
	...
	let style = {
		multiSelectColumn: { width: multiSelectWidth + 'px', padding: '9px', overflow:'hidden' },
		arrowColumn: { width: arrowCellWidth },
		dataColumn: { width: 'calc(' + columnPerc + '%' + ' - ' + columnPxToRemove + 'px )' },
	}				
