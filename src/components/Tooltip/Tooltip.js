	...
	render(){
		...
		return (
			<div className='element-tooltip' style={style} ref={box => this.box = box}>
				<div className='element-tooltip__content' ref={content => this.content = content}>{children}</div>
				<span className='element-tooltip__viewer' ref={viewer => this.viewer = viewer}>{children}</span>
			</div>
		)
	}
}
