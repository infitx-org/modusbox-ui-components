...
class Header extends React.Component{
	...
	render(){ 
		...
		const { id, hasChildren, hasMultiSelect, allSelected, someSelected, onMultiSelectAll, style, originalColumns, columns, filters, sortLabel, isSortingAsc, onColumnClick, onSearchClick, onSearchChange, onSearchRemove, onTriggerResizeWidth, onResizeWidth, showScrollbar } = this.props		
		return (
			<div className='data-list-header-row-box'>
				<Row className='data-list-header-row' style={{ paddingRight: showScrollbar ? '6px' : '0px'}}>
					...
					{ hasMultiSelect &&
						<div id={`${id}-multiselect-all`} className="data-list-header-cell data-list-header-column-cell" style={{...style.multiSelectColumn, paddingTop:6 }}>
							<Checkbox id='general-multi-select' isSelected={ allSelected } onChange={ onMultiSelectAll } halfChecked={ someSelected }/>
						</div> }			
					{
